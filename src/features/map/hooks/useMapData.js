...
/**
 * Validates a property's position data
...
const useMapData = (propertyCode = null) => {
  ...
  // Ref to track changes and prevent unnecessary updates
   ...
   * to prevent position drift across renders
  ...
  const createStableProperty = useCallback((property) => {
    ...
    // Normalize the position if it exists
    if (hasValidPosition(property)) {
      stableProperty.position = normalizePosition(property.position);
    }
  ...
  const fetchPropertyByCode = useCallback(async (code) => {
    ...
    try {
      ...
      const normalizedCenter = normalizePosition([
        stableProperty.position.lat,
        stableProperty.position.lng
      ]);
